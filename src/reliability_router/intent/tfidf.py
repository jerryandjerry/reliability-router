"""TF-IDF intent parser for exported sparse lexical classifiers.

A bag of character and word n-grams with a linear classifier on top. It is here as the cheap
baseline every heavier family has to beat: if lexical features alone route as well as an encoder,
the encoder is not earning its latency.

The concrete method (`tfidf_logreg`, `tfidf_svm`) comes from the exported manifest, so one runtime
module serves every variant.
"""
...
class TfidfIntentParser:
    """Load an exported TF-IDF pipeline and map requests to QUICK/DEEP decisions."""
    ...
    def __init__(self, config: RouterConfig, *, classifier: ProbabilityClassifier | None = None):
        ...
        self._classifier = classifier or self._load_classifier(self.model_path)
    ...
    def _load_manifest(model_path: Path) -> dict[str, Any]:
        ...
        if not manifest_path.exists():
            raise ValueError(f"TF-IDF manifest not found: {manifest_path}")
        loaded = json.loads(manifest_path.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise ValueError(f"TF-IDF manifest must be a JSON object: {manifest_path}")
        return loaded

    def _required_manifest_string(self, field: str) -> str:
        value = str(self.manifest.get(field) or "").strip()
        if not value:
            raise ValueError(f"TF-IDF manifest field {field!r} must be a non-empty string.")
        return value

    @staticmethod
    def _load_classifier(model_path: Path) -> ProbabilityClassifier:
        pipeline_path = model_path / "pipeline.joblib"
        if not pipeline_path.exists():
            raise ValueError(
                f"TF-IDF artifact not found: {pipeline_path}. "
                "Train/export a model under models/intent_tfidf/exported/<version> first."
            )
        try:
            import joblib
        except ImportError as exc:
            raise ImportError("TF-IDF intent parser requires: uv sync --extra tfidf") from exc

        pipeline = joblib.load(pipeline_path)

        def classify(text: str) -> float:
            # LogisticRegression exposes predict_proba; LinearSVC only a signed margin, so squash it
            if hasattr(pipeline, "predict_proba"):
                return float(pipeline.predict_proba([text])[0][1])
            margin = float(pipeline.decision_function([text])[0])
